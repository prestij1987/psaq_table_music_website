...
def get_album_by_artist(name):
    ...
    request_data = connection.execute(f'''SELECT col_nam FROM collections
                                             LEFT JOIN collection_album_track ON album_id = collection_album_track.album_id   
                                             WHERE id = '{name.lower()}'                         
                                             ''').fetchall()
...
SELECT albumname album, COUNT(trackname) track_count FROM album
JOIN track ON album.id = track.album.id
GROUP BY albumname
HAVING COUNT(trackname) = (
	SELECT COUNT(trackname) FROM album
	JOIN track ON album.id = track.album.id
	GROUP BY albumname
	ORDER BY COUNT(trackname)
	LIMIT 1);
